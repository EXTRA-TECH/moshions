...
const generateCart = (
  ...
  return items?.map(
    ({ image, quantity, variant, totalPrice, name, size }: any, i: number) => {
      return (
        <div
          key={variant?.id ? `id-${variant?.id}` : `idx-${i}`}
...
const Bag = ({ cartItemsQuantity }: any) => {
  const { loaded, items, removeItem, updateItem, subtotalPrice, totalPrice } =
    useCart();
  ...
  const { data: productVariants, refetch: refetchVariants } = useQuery(
    PRODUCT_VARIANTS,
    {
      variables: {
        ids,
    ...
    if (items?.length) {
      items?.map((item: any) => {
        for (let productLine in productVariants?.productVariants?.edges) {
          if (
            item?.variant?.id ===
            productVariants?.productVariants?.edges[productLine]?.node?.id
          ) {
            console.log(
              '>>..node',
              productVariants?.productVariants?.edges[productLine]?.node
            );
            item['name'] =
            ...
            item['image'] =
              productVariants?.productVariants?.edges[
                productLine
              ]?.node?.product?.thumbnail?.url;
            item['alt'] =
              productVariants?.productVariants?.edges[
                productLine
              ]?.node?.product?.thumbnail?.alt;
            item['size'] =
              productVariants?.productVariants?.edges[
                productLine
              ]?.node?.product?.thumbnail?.alt;
    ...
    <>
      <span className='header-cart-items'>
        <a
          ...
          />
          {cartItemsQuantity > 0 ? (
            <span className='cart-number'>{cartItemsQuantity}</span>
          ) : null}
        </a>
      </span>
      <div
        ...
        <div className='offcanvas-header cart-header text-light'>
          ...
          <div id='offcanvasRightLabe' style={{ marginTop: '5px' }}>
            <span className='h5 canvas-cart-title-text text-light'>
              MY CART
            </span>
          </div>
          <button
          ...
          {items?.length === 0 && (
            <div className='cart-empty'>
              ...
              <button
                ...
                className='btn text-reset cart-continue-shopping'
                data-bs-dismiss='offcanvas'
                aria-label='Close'>
                CONTINUE SHOPPING
              </button>
            ...
            <>
              ...
              <div className='continue-shopping py-3'>
                <button
                  ...
                  className='btn btn-light text-dark cart-continue-shopping'
                  data-bs-dismiss='offcanvas'
                  aria-label='Close'>
                  PROCEED TO CHECKOUT
                </button>
